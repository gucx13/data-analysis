...
//define pdf1
double poissonPdf1(double* xPtr, double* yPtr, double* zPtr, double par[]){
    double     s = *xPtr;
    double     b = *yPtr;
    double   sam = *zPtr;
    double    mu = fix_mu;  //
...
//  vector<double> SAM(pdf_sample, pdf_sample + n);
    ...
    double lnL = 0.0;
    for(int i = 0 ; i < n ; i++ ){
        double pdf = poissonPdf1(&pdf_sig[i],&pdf_bkg[i],&pdf_sample[i],par);
...
for (int i=0;i<nentries_sample;i++) {
	sample->GetEntry(i);
		if (energy_sample>1 && energy_sample<100){
...
for (int i=0;i<nentries_sig;i++) {
    sig->GetEntry(i);
        if (energy_sig>1 && energy_sig<100){
            hist_sig->Fill(energy_sig);
        }
    } 
for (int i=0;i<nentries_bkg;i++) {
    bkg->GetEntry(i);
        if (energy_bkg>1 && energy_bkg<100){
